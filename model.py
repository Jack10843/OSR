...
class TanhGaussianPolicy(nn.Module):
    ...
    def forward(self, observations, deterministic=False, repeat=None):
        ...
        mean, log_std = torch.split(base_network_output, self.action_dim, dim=-1)
        log_std = self.log_std_multiplier() * log_std + self.log_std_offset()
        return self.tanh_gaussian(mean, log_std, deterministic), mean, torch.exp(log_std)
...
class SamplerPolicy(object):
    ...
    def __call__(self, observations, deterministic=False):
        with torch.no_grad():
            ...
            pack, _, _ = self.policy(observations, deterministic)
            actions, _ = pack
...
class VAE(nn.Module):
    def __init__(self, input_dim, output_dim, device,
        ...
        self.d3 = nn.Linear(dim, output_dim * 2)
    ...
    def forward(self, observation, next_observation):
        ...
        u, m, s = self.decode2(observation, next_observation, z)
        return u[0], m, s, mean, std
