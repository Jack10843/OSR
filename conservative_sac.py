...
class ConservativeSAC(object):
    ...
    def __init__(self, config, policy, qf1, qf2, target_qf1, target_qf2, inverse_dynamics=None, device='cuda:0', beta=1e-2):
        ...
        if inverse_dynamics != None:
            self.inverse_dynamics = inverse_dynamics.to(self.device)
            self.id_optimizer = optimizer_class(
                self.inverse_dynamics.parameters(), 1e-4,
            )
    ...
    def train_inv(self, batch):
        observations = batch['observations']
        actions = batch['actions']
        next_observations = batch['next_observations']

        recon, _, _, mean, std = self.inverse_dynamics(observations, next_observations)
    ...
    def train(self, batch, bc=False):
        ...
        pi_pack, pi_mean, pi_std = self.policy(observations)
        new_actions, log_pi = pi_pack
        # recon, idm_mean, idm_std, _, _ = self.inverse_dynamics(observations, next_observations)
        ...
        observations_hat = observations.detach() + torch.randn(observations.shape).to(
            self.device).detach() * self.beta
        """ Policy loss """
        if bc:
            ...
            _, idm_mean, idm_std, _, _ = self.inverse_dynamics(observations_hat, next_observations)
            idm_mean = idm_mean.detach()
            idm_std = idm_std.detach()
            # # kl2
            _, pi_mean_hat, pi_std_hat = self.policy(observations_hat)
            pi_dist = Normal(pi_mean_hat, pi_std_hat)
            idm_dist = Normal(idm_mean, idm_std)
            osr_loss = torch.mean(kl_divergence(pi_dist, idm_dist))
            #
            policy_loss += 0.5 * torch.mean(osr_loss)
        ...
        if self.config.cql_max_target_backup:
            # pi_pack, pi_mean, pi_std = self.policy(observations)
            # new_actions, log_pi = pi_pack
            new_pi_pack, _, _ = self.policy(next_observations, repeat=self.config.cql_n_actions)
            new_next_actions, next_log_pi = new_pi_pack
            ...
            new_pi_pack, new_pi_mean, new_pi_std = self.policy(next_observations)
            new_next_actions, next_log_pi = new_pi_pack
        ...
        if not self.config.use_cql:
            ...
            cql_random_actions = actions.new_empty((batch_size, self.config.cql_n_actions, action_dim),
                                                   requires_grad=False).uniform_(-1, 1)
            pack_q, _, _ = self.policy(observations, repeat=self.config.cql_n_actions)
            cql_current_actions, cql_current_log_pis = pack_q
            pack_nq, _, _ = self.policy(next_observations, repeat=self.config.cql_n_actions)
            cql_next_actions, cql_next_log_pis = pack_nq
